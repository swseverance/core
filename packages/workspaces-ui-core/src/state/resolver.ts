...
export class LayoutStateResolver {
    ...
    public getFrameConstraints(): Constraints {
        ...
        const result = workspaceWrappers.reduce((acc, ww) => {
            ...
            const newMinWidth = Math.max(ww.minWidth + horizontalExtraSpace, acc.minWidth);
            const newMinHeight = Math.max(ww.minHeight + verticalExtraSpace, acc.minHeight);
            const newMaxWidth = Math.min(ww.maxWidth + horizontalExtraSpace, acc.maxWidth);
            const newMaxHeight = Math.min(ww.maxHeight + verticalExtraSpace, acc.maxHeight);
